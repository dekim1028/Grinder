...
const PlannerContainer = ({ history, match }) => {
	...
	const { planner, plannerError, checklist, loading } = useSelector(
		({ planner, checkList, loading }) => ({
	...
	const onChangeStudyTime = useCallback(
		(value) => {
			if (planner) {
				dispatch(
					changeField({
			...
			let studyTime = list
				...
				.reduce((ac, cu) => {
					...
					const startTime = new Date(
						plannerDateText + ' ' + cu.startTime,
					).getTime();
					const endTime = new Date(
						plannerDateText + ' ' + cu.endTime,
					).getTime();
