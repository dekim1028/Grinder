...
const UpdateCheclistContainer = ({ onConfirm }) => {
	...
	const { checklist, updateTarget, settings, planner } = useSelector(
		({ checkList, settings, planner }) => ({
	...
	const onUpdate = () => {
		...
		if (updateTarget.startTime !== '' && updateTarget.endTime !== '') {
			const startDateTime = new Date(
				moment(planner.date).format('yyyy.MM.DD') +
					' ' +
					updateTarget.startTime,
			).getTime();
			const endDateTime = new Date(
				moment(planner.date).format('yyyy.MM.DD') + ' ' + updateTarget.endTime,
			).getTime();
